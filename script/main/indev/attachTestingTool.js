const TESTING_TOOL = new SidebarTool({
	sidebarDescriptor: {
		fragments: [{
			icon: "support",
			fragments: [{
				...
				fragments: {
					...
					click(tool, fragment) {
						...
						fragment.setText("" + java.lang.String(text).replace(character, ""));
...
const attachTestingTool = function(post) {
	TESTING_TOOL.deattach();
	if (!TESTING_TOOL.isAttached()) {
		TESTING_TOOL.attach();
	}
	TESTING_TOOL.queue();
	handle(function() {
		TESTING_TOOL.collapse();
		let accepted = true;
		try {
			TESTING_TOOL.describe();
			post && post(TESTING_TOOL);
			accepted = false;
		} catch (e) {
			reportError(e);
		}
