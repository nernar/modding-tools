...
Unicode.registerCharset = function(chartOrName, start, end) {
	// if (typeof chartOrName != "object" || !(chartOrName instanceof Unicode.Charset)) {
...
ToolbarFragment.prototype.resetContainer = function() {
	...
	title.setTextSize(toComplexUnitDp(11));
...
ImageSourceFragment.prototype.setSource = function(pathOrFile) {
	let toolbar = this.getToolbar();
	toolbar.setTitle(Files.basename(pathOrFile = Files.of(pathOrFile), true));
	toolbar.setSubtitle(pathOrFile.getParentFile().getName() + "/");
	let view = this.getImageView();
	if (!(pathOrFile instanceof BitmapDrawable)) {
		pathOrFile = new BitmapDrawable(pathOrFile);
	}
	pathOrFile.attachAsImage(view);
...
const ImageSourceWindow = function() {
	...
	window.getFragment().setOnClickListener(function() {
		window.getFragment().requireControl();
	});
	window.getFragment().setOnBackClickListener(function() {
		window.dismiss();
	});
	window.getFragment().setOnScaleChangeListener(function() {
		switch (window.getFragment().getScaleType()) {
			case $.ImageView.ScaleType.CENTER:
	...
	window.getFragment().setBackground(ColorDrawable.parseColor("BLACK"));
...
const openImageFile = function(file) {
	let source = new this.ImageSourceWindow();
...
const attachAdvancedExplorer = function(path, mayWrap, action) {
	handle(function() {
		...
		bar.setOnOutsideListener(function(bar) {
			try {
				explorer.window.dismiss();
			} catch (e) {
				log("attachAdvancedExplorer#explorer.window: Not required anymore!");
				explorer.dismiss();
			}
		});
		...
		explorer.setOnSelectListener(function(popup, file) {
			let overriden = false;
			try {
				overriden = !!(action && action(explorer, file));
			} catch (e) {
				reportError(e);
			}
			if (!overriden) {
...
const openSupportedFileIfMay = function(file, type) {
	switch (type) {
		case Files.ExtensionType.ARCHIVE:
			confirm(translate("Warning!"), null, function() {
				let name = Files.basename(file, true);
				if (name == file.getName()) name += " unpacked";
				let output = Files.of(file.getParent(), name);
				if (output.exists()) {
					MCSystem.throwException("Modding Tools: Output file is already exists");
				}
		...
		default:
			...
			for (let key in Files.ExtensionType) {
				let requestedType = Files.ExtensionType[key];
				if (requestedType == "folder" || requestedType == "unknown" || requestedType == "none") continue;
				items.push(requestedType.charAt(0).toUpperCase() + requestedType.substring(1));
			...
			select(null, items, function(i) {
				openSupportedFileIfMay(file, types[i]);
			});
			break;
	}
};
