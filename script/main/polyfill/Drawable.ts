namespace BitmapFactory {
	export const __decodeFile = BitmapFactory.decodeFile;

	/**
	 * @requires `isAndroid()`
	 */
	export function createCompressed(bitmap, min, max) {
		...
		return BitmapFactory.createScaled(bitmap, dx, dy);
	}

	/**
	 * @requires `isAndroid()`
	 */
	export function getUpscaleRatio(width, height, dx, dy) {
	 ...
	 * @requires `isAndroid()`
	 */
	export function getBoundsDecodeOptions() {
	 ...
	 * @requires `isAndroid()`
	 */
	export function getPotatoOptions() {
		let options = new android.graphics.BitmapFactory.Options();
		options.inPreferredConfig = android.graphics.Bitmap.Config.ARGB_4444;
		return options;
	}

	export function decodeResource(bytes, options) {
		try {
			// @ts-expect-error
			let decoded = decodeResource(bytes);
			return BitmapFactory.decodeBytes(decoded, options);
		} catch (e) {
...
BitmapFactory.decodeFile = (pathOrFile, options?) => {
	pathOrFile = Files.of(pathOrFile);
	try {
		if (Files.extension(pathOrFile) == "dnr") {
			let bytes = Files.readAsBytes(pathOrFile);
			return BitmapFactory.decodeResource(bytes, options);
		}
		return BitmapFactory.__decodeFile(pathOrFile, options);
	} catch (e) {
...
namespace ImageFactory {
	 ...
	 * @requires `isAndroid()`
	 */
	export function clipAndMerge(background, foreground, level, orientate?) {
...
AnimationDrawable.applyDescribe = (drawable, json) => {
	if (drawable == null) {
	...
	export function parseJson(instanceOrJson, json?) {
		...
		if (Array.isArray(json)) {
			for (let index = 0; index < json.length; index++) {
				let frame = calloutOrParse(json, json[index], [this, instanceOrJson]);
...
Drawable.applyDescribe = (drawable, json) => {
	if (drawable === null) {
...
namespace ColorDrawable {
	export const __parseColor = ColorDrawable.parseColor;
}

ColorDrawable.parseColor = (color, inBackground?) => {
	if (isAndroid()) {
...
const fetchResourceOverridesIfNeeded = (path) => {
	try {
...
namespace BitmapDrawableFactory {
	export const __mapDirectory = BitmapDrawableFactory.mapDirectory;
}

BitmapDrawableFactory.mapDirectory = (path, explore, root) => {
	fetchResourceOverridesIfNeeded(path);
	return BitmapDrawableFactory.__mapDirectory(path, explore, root);
};
