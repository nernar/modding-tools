const Dirs: {
	...
	ASSET: string,
	PROJECT: string,
	PROJECT_AUTOSAVE: string,
	LOGGING: string,
	SCRIPT: string,
	SCRIPT_ADAPTIVE: string,
...
namespace Files {
	export function mime(pathOrFile) {
	 ...
	 */
	export function prepareSize(pathOrFile) {
		return this.prepareFormattedSize(this.of(pathOrFile).length());
	}

	export function prepareFormattedSize(size) {
	 ...
	 * @requires `isAndroid()`
	 */
	export function createTypefaceWithFallback(pathOrFile) {
